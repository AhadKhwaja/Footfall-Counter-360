...
import { XAxis, YAxis, CartesianGrid, LabelList, Tooltip, Legend, BarChart, Bar } from 'recharts';
...
export const WeeklyTraffic = () => {
    const { week_traffic, dark_mode } = useContext(StateContext);
    const [totalVisitors, setTotalVisitors] = useState(0);
    const [bestWeak, setBestWeek] = useState(0);
    const [worstWeek, setWorstWeek] = useState(0);
    const weekTraffic = week_traffic[0];
    const darkMode = dark_mode[0];
    const [peakWeek, setPeakWeek] = useState('');
    const [leastWeek, setLeastWeek] = useState('');

    useEffect(() => {
        let total = 0;
        let data = [...weekTraffic];
        let visitorArr = [];
        if (data.length) {
            data.map(element => {
            ...
            setBestWeek(Math.max(...visitorArr));
            setWorstWeek(Math.min(...visitorArr));
            setPeakWeek(data[indexP].Week);
            setLeastWeek(data[indexL].Week);
    ...
    return (
        <div className="graph-grid-container">
            <div className={darkMode ? "trafficGraphDark" : "trafficGraph"}>
                <div id="graph-header"> <h3 className={darkMode ? "graph_title_dark" : "graph_title"}>Total Traffic</h3><DateTabTraffic /><CSVLink id="csvLink" data={weekTraffic}><img id="csvImage" src={csv} alt={'csv'} /></CSVLink></div>
                <BarChart width={800} height={250} data={weekTraffic} barSize={50} barCategoryGap='15%'
                    margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 6" vertical={false} />
                    <XAxis dataKey="Week" padding={{ left: 20, right: 20 }} tickLine={false} axisLine={false} tick={darkMode ? <CustomizedXAxisTickDark /> : <CustomizedXAxisTick />} />
            ...
            <div id={darkMode ? "total_visitors_dark" : "total_visitors"}>
                ...
                <span id={darkMode ? "visitors_heading_dark" : "visitors_heading"}>{bestWeak}</span>
                <span id={darkMode ? "visitors_subHeading_dark" : "visitors_subHeading"}>{peakWeek}</span>
                <span id={darkMode ? "visitor_subsubHeading_dark" : "visitor_subsubHeading"}>Best Performing Week</span>
                <span id={darkMode ? "visitors_heading_dark" : "visitors_heading"}>{worstWeek}</span>
                <span id={darkMode ? "visitors_subHeading_dark" : "visitors_subHeading"}>{leastWeek}</span>
                <span id={darkMode ? "visitor_subsubHeading_dark" : "visitor_subsubHeading"}>Least Performing Week</span>
            </div>
        </div>
    );
};

export const WeeklyGender = () => {

    const { week_traffic, age_pie_graph, dark_mode } = useContext(StateContext);
    const weekTraffic = week_traffic[0];
    const darkMode = dark_mode[0];
    ...
    return (
        <div className="graph-grid-container">
            <div className={darkMode ? "gender_graph_week_dark" : "gender_graph"}>
                <div id="graph-header"> <h3 className={darkMode ? "graph_title_dark" : "graph_title"}>Gender Distribution</h3><DateTabGender /><CSVLink id="csvLink" data={weekTraffic}><img id="csvImage" src={csv} alt={'csv'} /></CSVLink></div>
                <BarChart style={barChartStyle1} width={800} height={250} data={weekTraffic} barSize={40}
                    margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <XAxis dataKey="Week" padding={{ left: 20, right: 20 }} tickLine={false} axisLine={false} tick={darkMode ? <CustomizedXAxisTickDark /> : <CustomizedXAxisTick />} />
                    <YAxis type="number" tickLine={false} axisLine={false} tick={darkMode ? <CustomizedYAxisTickDark /> : <CustomizedYAxisTick />} />
                    <CartesianGrid strokeDasharray="3 6" vertical={false} />
                    <Tooltip cursor={{ fill: 'transparent' }} />
                    {/* <Legend /> */}
                    <Bar type="monotone" dataKey="Female" fill='#fc7cf0' >
                        <LabelList dataKey="Female" content={renderCustomizedLabel} />
                    </Bar>
                    <Bar type="monotone" dataKey="Male" fill='rgb(0, 174, 255)' >
    ...
    return (
        <div className="graph-grid-container">
            <div className={darkMode ? "age_graph_dark" : "age_graph"}>
                <div id="graph-header"> <h3 className={darkMode ? "graph_title_dark" : "graph_title"}>Age Distribution</h3><DateTabAge /><CSVLink id="csvLink" data={weekTraffic}><img id="csvImage" src={csv} alt={'csv'} /></CSVLink></div>
                <BarChart width={1160} height={300} data={weekTraffic} barSize={40}
                    margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <XAxis dataKey="Week" padding={{ left: 20, right: 20 }} tickLine={false} axisLine={false} tick={darkMode ? <CustomizedXAxisTickDark /> : <CustomizedXAxisTick />} />
                    <YAxis type="number" tickLine={false} axisLine={false} tick={darkMode ? <CustomizedYAxisTickDark /> : <CustomizedYAxisTick />} />
                    <CartesianGrid strokeDasharray="3 6" vertical={false} />
