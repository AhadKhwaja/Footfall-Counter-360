...
export const QuarterlyTraffic = () => {
    const { dark_mode, quarter_traffic } = useContext(StateContext);
    const [totalVisitors, setTotalVisitors] = useState(0);
    const [bestQuarter, setBestQuarter] = useState(0);
    const [worstQuarter, setWorstQuarter] = useState(0);
    const darkMode = dark_mode[0];
    const quarterTraffic = quarter_traffic[0];
    const [peakQuarter, setPeakQuarter] = useState('');
    const [leastQuarter, setworstQuarter] = useState('');

    useEffect(() => {
        let total = 0;
        let data = [...quarterTraffic];
        let visitorArr = [];
        if (data.length) {
            data.map(element => {
            ...
            setBestQuarter(Math.max(...visitorArr));
            setWorstQuarter(Math.min(...visitorArr));
            setPeakQuarter(data[indexP].quarter);
            setworstQuarter(data[indexL].quarter);
    ...
    return (
        <div className="graph-grid-container">
            <div className={darkMode ? "trafficGraphDark" : "trafficGraph"}>
                <div id="graph-header"> <h3 className={darkMode ? "graph_title_dark" : "graph_title"}>Total Traffic</h3><DateTabTraffic /><CSVLink id="csvLink" data={quarterTraffic}><img id="csvImage" src={csv} alt={'csv'} /></CSVLink></div>
                <BarChart width={800} height={250} data={quarterTraffic} barSize={50} barCategoryGap='15%'
                    margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 6" vertical={false} />
                    <XAxis dataKey="quarter" padding={{ left: 20, right: 20 }} tickLine={false} axisLine={false} tick={darkMode ? <CustomizedXAxisTickDark /> : <CustomizedXAxisTick />} />
            ...
            <div id={darkMode ? "total_visitors_dark" : "total_visitors"}>
                ...
                <span id={darkMode ? "visitors_heading_dark" : "visitors_heading"}>{bestQuarter}</span>
                <span id={darkMode ? "visitors_subHeading_dark" : "visitors_subHeading"}>{peakQuarter}</span>
                <span id={darkMode ? "visitor_subsubHeading_dark" : "visitor_subsubHeading"}>Best Performing Quarter</span>
                <span id={darkMode ? "visitors_heading_dark" : "visitors_heading"}>{worstQuarter}</span>
                <span id={darkMode ? "visitors_subHeading_dark" : "visitors_subHeading"}>{leastQuarter}</span>
                <span id={darkMode ? "visitor_subsubHeading_dark" : "visitor_subsubHeading"}>Least Performing Quarter</span>
            </div>
        </div>
    )
};

export const QuarterlyGender = () => {
    const { quarter_traffic, total_gender_mq, age_pie_graph, dark_mode } = useContext(StateContext);
    const darkMode = dark_mode[0];
    ...
    return (
        <div className="graph-grid-container">
            <div className={darkMode ? "gender_graph_week_dark" : "gender_graph"}>
                <div id="graph-header"> <h3 className={darkMode ? "graph_title_dark" : "graph_title"}>Gender Distribution</h3><DateTabGender /><CSVLink id="csvLink" data={quarterTraffic}><img id="csvImage" src={csv} alt={'csv'} /></CSVLink></div>
                <BarChart style={barChartStyle1} width={800} height={250} data={quarterTraffic} barSize={40}
                    margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <XAxis dataKey="quarter" padding={{ left: 20, right: 20 }} tickLine={false} axisLine={false} tick={darkMode ? <CustomizedXAxisTickDark /> : <CustomizedXAxisTick />} />
                    <YAxis type="number" tickLine={false} axisLine={false} tick={darkMode ? <CustomizedYAxisTickDark /> : <CustomizedYAxisTick />} />
                    <CartesianGrid strokeDasharray="3 6" vertical={false} />
                    <Tooltip cursor={{ fill: 'transparent' }} />
                    <Bar type="monotone" dataKey="females" fill='#fc7cf0' >
                        <LabelList dataKey="females" content={renderCustomizedLabel} />
                    </Bar>
                    <Bar type="monotone" dataKey="males" fill='rgb(0, 174, 255)' >
...
export const QuarterlyAge = () => {
    const { quarter_traffic, dark_mode } = useContext(StateContext);
    const quarterTraffic = quarter_traffic[0];
    const darkMode = dark_mode[0];
    ...
    return (
        <div className="graph-grid-container">
            <div className={darkMode ? "age_graph_dark" : "age_graph"}>
                <div id="graph-header"> <h3 className={darkMode ? "graph_title_dark" : "graph_title"}>Age Distribution</h3><DateTabAge /><CSVLink id="csvLink" data={quarterTraffic}><img id="csvImage" src={csv} alt={'csv'} /></CSVLink></div>
                <BarChart width={1160} height={300} data={quarterTraffic} barSize={40}
                    margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <XAxis dataKey="quarter" padding={{ left: 20, right: 20 }} tickLine={false} axisLine={false} tick={darkMode ? <CustomizedXAxisTickDark /> : <CustomizedXAxisTick />} />
                    <YAxis type="number" tickLine={false} axisLine={false} tick={darkMode ? <CustomizedYAxisTickDark /> : <CustomizedYAxisTick />} />
                    <CartesianGrid strokeDasharray="3 6" vertical={false} />
